...
class BPU extends Module {
    ...
    //allocate ptr/victem ptr, consider btb as a queue, allocate from tail, victem from head
    val btb_victem_ptr = RegInit(0.U(idx_len.W))
    ...
    1.when didn't make a prediction(no entry in btb, aka default as not taken), and the branch is taken, then allocate a new entry in btb when resolved.
        ...
        if given a more carefully designed victem algorithm, we can mark the not takens invalid entry for other entries to use
    ...
    4.another case is, when a branch is predicted, the entry in the btb is valid, but when resolved, the entry has became a victem
    ...
    */
    ...
    when(io.i_branch_resolve_pack.valid && !io.i_branch_resolve_pack.prediction_valid && io.i_branch_resolve_pack.taken){
        btb(btb_victem_ptr).valid := true.B
        btb(btb_victem_ptr).tag := io.i_branch_resolve_pack.pc(12,3)//fetchpack address
        btb(btb_victem_ptr).target_address := io.i_branch_resolve_pack.target
        btb(btb_victem_ptr).branch_type := io.i_branch_resolve_pack.branch_type//??
        btb(btb_victem_ptr).select := io.i_branch_resolve_pack.pc(3) //if it's aligned to fetchpack address
        btb(btb_victem_ptr).bht := Mux(io.i_branch_resolve_pack.taken, 1.U(2.W), 0.U(2.W))

        //update victem ptr
        btb_victem_ptr := btb_victem_ptr + 1.U
    }
    //update on a mispredicted branch,暗含valid表示是一个branch,mispred代表即使没有pred,也可能会mispred
    when(io.i_branch_resolve_pack.valid && io.i_branch_resolve_pack.mispred && io.i_branch_resolve_pack.valid){
            for(i <- 0 until btb_size){
                when(btb(i).tag === io.i_branch_resolve_pack.pc(9,0)){
                    assert(btb(i).tag === io.i_branch_resolve_pack.pc(12,3),"btb entry not found on a br resolve and valid prediction\n")
